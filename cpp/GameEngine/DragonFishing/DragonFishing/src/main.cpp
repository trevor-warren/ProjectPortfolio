...
int main(int argc, char* argv[])
{
  // Initialization
	try
	{
		...
		// Creating and initializing the audio engine
		new Audio::AudioEngine();
		Audio::AUDIO_ENGINE->Initialize();

		// Creating the CDU because I forgot
		new Engine::CollisionDetectionUnit;
	}
  ...
  // Engine Start
	{
		...
		while (!window.IsClosed())
		{
			...
			Audio::AUDIO_ENGINE->Update(delta);
			Engine::CDU->Update();
  ...
  // Shutdown
	try
	{
		...
		Audio::AUDIO_ENGINE->Shutdown();
		delete Audio::AUDIO_ENGINE;
